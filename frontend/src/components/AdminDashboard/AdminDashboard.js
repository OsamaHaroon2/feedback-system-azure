...
const AdminDashboard = () => {
  ...
  const handleRetrieveFeedback = async () => {
    ...
    try {
      ...
      showToast("error", "Error retrieving feedback.");
    } finally {
      setLoading(false);
    }
  };
  ...
  // React.useEffect(() => {
  //   updateReportsList(); // Fetch reports list on initial load
  // }, []);
  ...
  return (
    <div className="admin-dashboard" style={{ padding: '20px', backgroundColor: '#f7f7f7' }}>
      ...
      <Spin spinning={loading}>
        <Row gutter={16}>
          <Col span={24}>
            <Card title="Filters & Actions" bordered={false} style={{ borderRadius: '8px', boxShadow: '0 2px 8px rgba(0, 0, 0, 0.1)', backgroundColor: '#ffffff' }}>
              <FeedbackFilters filters={filters} onFilterChange={handleFilterChange} />
              <div className="admin-actions" style={{ marginTop: '20px' }}>
                <button className="border p-2 rounded-lg border-teal-500 hover:bg-teal-500 hover:text-white" style={{ marginRight: '10px' }} onClick={handleRetrieveFeedback}>
                  Retrieve Feedback
                </button>
                <button className="border p-2 rounded-lg border-teal-500 hover:bg-teal-500 hover:text-white" style={{ marginRight: '10px' }} onClick={() => handleGenerateReport(false)}>
                  Generate All Subjects Report
                </button>
                <button className="border p-2 rounded-lg border-teal-500 hover:bg-teal-500 hover:text-white" style={{ marginRight: '10px' }} onClick={() => handleGenerateReport(true)}>
                  Generate Specific Subject Report
                </button>
                <button  className="border p-2 rounded-lg border-teal-500 hover:bg-teal-500 hover:text-white"  onClick={updateReportsList}>Retrieve Reports</button>
              </div>
        ...
        <Row gutter={16}>
          <Col span={12}>
            <Card title="Feedback Data" bordered={false} style={{ marginTop: '20px', borderRadius: '8px', boxShadow: '0 2px 8px rgba(0, 0, 0, 0.1)', backgroundColor: '#ffffff' }}>
          ...
          <Col span={12}>
            <Card title="Available Reports" bordered={false} style={{ marginTop: '20px', borderRadius: '8px', boxShadow: '0 2px 8px rgba(0, 0, 0, 0.1)', backgroundColor: '#ffffff' }}>
